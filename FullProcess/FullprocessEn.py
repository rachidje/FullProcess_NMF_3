...
class FullProcessText:
    ...
    def correction_err(self, mot):
        if mot in self.mots_erreur[0]:
            return mot[:-1] # delete last char
        elif mot in self.mots_erreur[1]:
            return mot[:-2] # delete last 2 char
        elif mot in self.mots_erreur[2]:
            return mot + "s" # ass "s" at the end
        else :
            return mot
    ...
    def correction_err_ngram(self, mot):
        if mot in self.mots_erreur_ngram:
            return mot + "s"
        else :
            return mot
    ...
    def fullProcess(self, text):
        new_text = text.replace("of course","of_course")
        new_text = new_text.lower().replace("ku klux klan","ku_klux_klan")

        new_text = self.spacyProcess(str(new_text).lower())
        wordslist = [self.correction_err(word) for word in str(new_text).split()]

        new_wordslist = self.compute_wordlists_ngram2(wordslist)
        new_wordslist = [self.correction_err_ngram(word) for word in new_wordslist]
